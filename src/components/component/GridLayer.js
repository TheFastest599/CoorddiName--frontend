...
const GridLayer = ({ gridSize = 1, trigZoom = 14 }) => {
  const map = useMap();
  ...
  const drawGrid = useCallback(() => {
    ...
    for (let i = 0; i < totalLatLines; i++) {
      const latDecimal = toDecimal({
        degrees: latStart + Math.floor(i / (3600 / gridSize)),
        minutes: Math.floor((i % (3600 / gridSize)) / (60 / gridSize)),
        seconds: ((i % (3600 / gridSize)) % (60 / gridSize)) * gridSize,
      });
    ...
    for (let i = 0; i < totalLngLines; i++) {
      const lngDecimal = toDecimal({
        degrees: lngStart + Math.floor(i / (3600 / gridSize)),
        minutes: Math.floor((i % (3600 / gridSize)) / (60 / gridSize)),
        seconds: ((i % (3600 / gridSize)) % (60 / gridSize)) * gridSize,
      });
  ...
  return (
    <>
      {gridLines.map((line, index) => (
        <Polyline key={index} positions={line} color="gray" weight={0.5} />
      ))}
    </>
  );
};
